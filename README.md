<div align="center">

<!-- Header Banner -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:2c5364&height=200&section=header&text=Ved%20Prakash%20Shukla&fontSize=50&fontColor=ffffff&fontAlignY=38&desc=Data%20Engineer%20%7C%204%20Years%20of%20Experience%20%7C%20India&descAlignY=58&descSize=18&animation=fadeIn" />

<!-- Profile Views & Social Badges -->
<p>
  <img src="https://komarev.com/ghpvc/?username=vedpshukla&label=Profile%20Views&color=2c5364&style=for-the-badge" alt="Profile Views" />
  <a href="https://www.linkedin.com/in/vedshuklaknp">
    <img src="https://img.shields.io/badge/LinkedIn-vedshuklaknp-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:vedp.motigarpur@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-vedp.motigarpur@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/vedpshukla">
    <img src="https://img.shields.io/badge/GitHub-vedpshukla-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

</div>

---

## 👨‍💻 About Me

> *"Designing resilient data pipelines and transforming raw data into actionable business intelligence."*

I'm a **Data Engineer with 4 years of experience** in designing, implementing, and optimizing robust data pipelines and ETL processes. Proficient in **Apache Spark, Databricks, Azure Data Factory, Delta Lake, SQL, and Python**, I have a proven track record of leveraging cloud platforms (Azure & AWS) and data warehousing principles to ensure data reliability and deliver critical business intelligence solutions.

- 🔭 **Currently working at:** EXL, Noida — building scalable data pipelines with Spark & Databricks
- 🌱 **Continuously learning:** Advanced Data Engineering, Cloud-native architectures
- 👯 **Open to collaborate on:** AI, Data Engineering & Cloud Projects
- 💬 **Ask me about:** SQL, Python, PySpark, Azure Data Factory, ETL Design
- 📫 **Reach me at:** [vedp.motigarpur@gmail.com](mailto:vedp.motigarpur@gmail.com) | 📱 +91-8795900521
- 😄 **Pronouns:** He/Him

---

## 💼 Work Experience

### 🏢 EXL, Noida — *Data Engineer* &nbsp; `Jan 2024 – Present`
- Designed and implemented **scalable data pipelines** integrating diverse sources using **Apache Spark, Databricks, and SQL**
- Orchestrated complex **ETL workflows** with **Azure Data Factory**, enhancing automation across multi-environment deployments
- Established **Delta Lake** transactions ensuring data reliability and integrity within the data lake architecture
- Proactively managed pipeline issues via ADF monitoring, reducing downtime and ensuring data availability

### 🏢 Paytm, Noida — *Data Analyst* &nbsp; `May 2023 – Jan 2024`
- Automated employee financial data adjustments, reducing processing time by **50%** for 40,000+ employees
- Developed complex credit column adjustment logic and output file generator, increasing efficiency by **40%**

### 🏢 De Soto Technologies, Ahmedabad — *Data Analyst* &nbsp; `Jan 2023 – Apr 2023`
- Developed and maintained ETL processes for accurate data collection from 3 diverse sources
- Automated data extraction from **Google Analytics to Redshift**, reducing reporting time by **60%**

### 🏢 SalescodeAI, Gurgaon — *L2 Engineer* &nbsp; `July 2021 – Jan 2023`
- Optimized SQL queries, reducing data processing time by **30%** and improving application responsiveness
- Analyzed data using **Elasticsearch, Kafka, and AWS logs**; automated daily reports using Advanced Excel

---

## 🛠️ Tech Stack

<p align="left">
  <img src="https://img.icons8.com/fluency/48/python.png" title="Python" alt="Python" />&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" title="MySQL" alt="MySQL" width="48" height="48" />&nbsp;
  <img src="https://img.icons8.com/color/48/mongodb.png" title="MongoDB" alt="MongoDB" />&nbsp;
  <img src="https://img.icons8.com/color/48/amazon-web-services.png" title="AWS" alt="AWS" />&nbsp;
  <img src="https://img.icons8.com/color/48/microsoft-azure.png" title="Azure" alt="Azure" />&nbsp;
  <img src="https://img.icons8.com/color/48/power-bi.png" title="Power BI" alt="Power BI" />&nbsp;
  <img src="https://img.icons8.com/color/48/tableau-software.png" title="Tableau" alt="Tableau" />&nbsp;
  <img src="https://img.icons8.com/color/48/git.png" title="Git" alt="Git" />&nbsp;
</p>

| Category | Technologies |
|---|---|
| **Languages** | Python (NumPy, Pandas), SQL, PySpark |
| **Databases & Warehousing** | MySQL, PostgreSQL, SQL Server, Oracle, MongoDB, Redshift, Elasticsearch, Delta Lake |
| **Big Data** | Apache Spark, Apache Kafka, Databricks, SparkSQL |
| **Cloud Platforms** | Microsoft Azure (Azure Data Factory), AWS |
| **BI & Visualization** | Tableau, Power BI, Advanced Excel |
| **Tools** | Git, Postman, ETL, Query Optimization, EDA, Data Transformation |

---

## 🎓 Education

**Kanpur Institute of Technology, Kanpur, India** &nbsp; `2017 – 2021`  
B.Tech — Computer Science and Engineering | **CGPA: 7.3**

---

## 📊 GitHub Stats

<p align="center">
  <a href="https://github.com/vedpshukla/">
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=vedpshukla&langs_count=6&theme=tokyonight&layout=compact&hide_border=true" alt="Top Languages" />
  </a>
</p>

<p align="center">
  <a href="https://github.com/vedpshukla/">
    <img width="49.5%" src="https://github-readme-stats.vercel.app/api?username=vedpshukla&show_icons=true&theme=tokyonight&hide_border=true" alt="GitHub Stats" />
    <img width="49.5%" src="https://github-readme-streak-stats.herokuapp.com/?user=vedpshukla&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
  </a>
</p>

---

<div align="center">

### 🤝 Let's Connect & Build Something Great!

<a href="https://www.linkedin.com/in/vedshuklaknp">
  <img src="https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
&nbsp;
<a href="mailto:vedp.motigarpur@gmail.com">
  <img src="https://img.shields.io/badge/-Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
&nbsp;
<a href="https://github.com/vedpshukla">
  <img src="https://img.shields.io/badge/-GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br/><br/>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:2c5364,50:203a43,100:0f2027&height=100&section=footer" />

</div>
